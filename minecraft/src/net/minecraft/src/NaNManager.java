...
public final class NaNManager {
	...
	public static void tick(Minecraft mc) {
		...
		if(--ticksUntilEvent <= 0) {
			startEffect(mc, nextEffect, nextEffect == EFFECT_FRAME_BLEED ? 20 * 7 : 20 * 5);
			nextEffect = nextEffect == EFFECT_FRAME_BLEED ? EFFECT_VOXEL_COLLAPSE : EFFECT_FRAME_BLEED;
			ticksUntilEvent = nextInterval();
		}
	}
	...
	private static void startEffect(Minecraft mc, int effectId, int durationTicks) {
		...
		if(isActive(effectId)) mc.sndManager.playSoundFX("glitch.glitch", 1.0F, 1.0F);
	}

	public static void beginEffect(int effectId, int durationTicks) {
		if(effectId < EFFECT_VOXEL_COLLAPSE || effectId > EFFECT_FRAME_BLEED) return;
	...
	public static String debugCommand(Minecraft mc, String command) {
		...
		if("event".equalsIgnoreCase(parts[0]) && parts.length > 1) {
			...
			startEffect(mc, effect, effect == EFFECT_FRAME_BLEED ? 20 * 7 : 20 * 5);
	...
	public static boolean hideHud() {
		return activeEffect == EFFECT_VOXEL_COLLAPSE || activeEffect == EFFECT_FRAME_BLEED;
	}

	public static void renderEffectOverlay(int width, int height) {
		if(!hideHud()) return;
		GL11.glPushMatrix();
		...
		if(activeEffect == EFFECT_VOXEL_COLLAPSE) {
