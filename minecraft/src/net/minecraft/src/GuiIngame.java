...
public class GuiIngame extends Gui {
        ...
        public void renderGameOverlay(float var1, boolean var2, int var3, int var4) {
                ...
                // Render scary messages during final event
                if(this.mc.finalEventActive) {
                        long elapsedMs = System.currentTimeMillis() - this.mc.finalEventStartTime;
                        if(elapsedMs < 19000) { // Show messages for first 19 seconds
                                // Draw random scary messages at random positions on screen
                                for(int i = 0; i < 5; i++) {
                                        int msgX = this.mc.random.nextInt(var6);
                                        int msgY = this.mc.random.nextInt(var7);
                                        String randomMsg = this.mc.scaryMessages[this.mc.random.nextInt(this.mc.scaryMessages.length)];
                                        var8.drawString(randomMsg, msgX, msgY, 0xFFFF0000);
                                }
                        }
                }
                ...
                if(this.mc.gameSettings.showDebugInfo) {
                        var8.drawStringWithShadow("Minecraft NaN (" + this.mc.debug + ")", 2, 2, 16777215);
                        ...
                        var8.drawStringWithShadow("Minecraft NaN", 2, 2, 16777215);
                }
        ...
        public void setRecordPlayingMessage(String var1) {
                this.recordPlaying = "PLAYING!!!!!!!! : " + var1;
