...
public class ConsoleCommandHandler {
	...
	public void handleCommand(ServerCommand var1) {
		...
		if(!var2.toLowerCase().startsWith("help") && !var2.toLowerCase().startsWith("?")) {
			if(var2.toLowerCase().startsWith("mst") || var2.toLowerCase().startsWith("event") || var2.toLowerCase().startsWith("next") || var2.toLowerCase().matches("x([0-9]+|\\s+[0-9]+)") || var2.toLowerCase().startsWith("anticlose")) {
	...
	private void printHelp(ICommandListener var1) {
		...
		var1.log("   mst                       shows the next NaN event");
		var1.log("   event <name>              starts a named NaN event");
		var1.log("   next                      starts the next NaN event in order");
	}
